...
pub fn part_one(input: &str) -> i64 {
  ...
  for i in 0..combos.len() {
    if people_dupes.contains(&combos[i].0)
    || people_dupes.contains(&combos[i].1) {
      continue;
    }
    combo = combos.remove(i);
    seating.push(combo);
...
mod day12_tests {
  ...
  #[test]
  fn samples_part1() {
    assert_eq!(part_one("Alice would gain 54 happiness units by sitting next to Bob.
  ...
  #[test]
  fn parse_line_test() {
    assert_eq!(parse_line("Alice would gain 54 happiness units by sitting next to Bob."), ("Alice", "Bob", 54));
    assert_eq!(parse_line("Alice would lose 79 happiness units by sitting next to Carol."), ("Alice", "Carol", -79));
    assert_eq!(parse_line("Alice would lose 2 happiness units by sitting next to David."), ("Alice", "David", -2));
    assert_eq!(parse_line("Bob would gain 83 happiness units by sitting next to Alice."), ("Bob", "Alice", 83));
    assert_eq!(parse_line("Bob would lose 7 happiness units by sitting next to Carol."), ("Bob", "Carol", -7));
    assert_eq!(parse_line("Bob would lose 63 happiness units by sitting next to David."), ("Bob", "David", -63));
    assert_eq!(parse_line("Carol would lose 62 happiness units by sitting next to Alice."), ("Carol", "Alice", -62));
    assert_eq!(parse_line("Carol would gain 60 happiness units by sitting next to Bob."), ("Carol", "Bob", 60));
    assert_eq!(parse_line("Carol would gain 55 happiness units by sitting next to David."), ("Carol", "David", 55));
    assert_eq!(parse_line("David would gain 46 happiness units by sitting next to Alice."), ("David", "Alice", 46));
    assert_eq!(parse_line("David would lose 7 happiness units by sitting next to Bob."), ("David", "Bob", -7));
    assert_eq!(parse_line("David would gain 41 happiness units by sitting next to Carol."), ("David", "Carol", 41));
  }
}
